layui.use(['layer', 'form', 'table', 'ztree', 'laydate', 'admin', 'ax','jquery'], function () {
    ...
    MgrUser.onDeleteUser = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.apiPath + "/mgr/delete", function () {
    ...
    MgrUser.resetPassword = function (data) {
        Feng.confirm("是否重置密码为111111 ?", function () {
            var ajax = new $ax(Feng.apiPath + "/mgr/reset", function (data) {
                Feng.success("重置密码成功!");
            }, function (data) {
                Feng.error("重置密码失败!");
            });
    ...
    MgrUser.changeUserStatus = function (userId, checked) {
        if (checked) {
            var ajax = new $ax(Feng.apiPath + "/mgr/unfreeze", function (data) {
                Feng.success("解除冻结成功!");
            }, function (data) {
                Feng.error("解除冻结失败!");
                table.reload(MgrUser.tableId);
            ...
            var ajax = new $ax(Feng.apiPath + "/mgr/freeze", function (data) {
                Feng.success("冻结成功!");
            }, function (data) {
    ...
    //初始化左侧部门树
    var ztree = new $ZTree("deptTree", Feng.apiPath+"/dept/tree");
