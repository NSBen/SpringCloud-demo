layui.use(['layer', 'admin','table', 'ax', 'laydate'], function () {
    var $ = layui.$;
    var $ax = layui.ax;
    ...
    LoginLog.logDetail = function (param) {
        var ajax = new $ax(Feng.apiPath + "/log/detail/" + param.operationLogId, function (data) {
    ...
    LoginLog.cleanLog = function () {
        Feng.confirm("是否清空所有日志?", function () {
            var ajax = new $ax(Feng.apiPath + "/log/delLog", function (data) {
                Feng.success("清空日志成功!");
                LoginLog.search();
            }, function (data) {
                Feng.error("清空日志失败!");
